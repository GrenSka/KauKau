...
app.post('/message', (req, res) => {
  if (req.session.user === 'KauKau<33') {
    ...
    res.send('Only KauKau<33 can send messages 💌');
  }
});
...
app.get('/messages', (req, res) => {
  if (req.session.user === 'KauKau<33') {
    const visibleMessages = messages.filter(m => m.user !== 'KauKau<33');
